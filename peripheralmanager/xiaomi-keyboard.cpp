...
int calculateAngle(float kX, float kY, float kZ, float pX, float pY, float pZ) {
  ...
  for (int i = 0; i <= 360; i++) {
    if (i > 90) {
      if (i > 90 && i <= 180) {
        float deviation12 =
            (((-angle_cos[180 - i]) * kX2) - (angle_sin[180 - i] * kZ2)) + pX2;
        deviation2 =
            ((-angle_cos[180 - i]) * kZ2) + (angle_sin[180 - i] * kX2) + pZ2;
        deviation1 = deviation12;
      } else if (i > 180 && i <= 270) {
        float deviation13 =
            (((-angle_cos[i - 180]) * kX2) - ((-angle_sin[i - 180]) * kZ2)) +
            pX2;
        deviation2 =
            ((-angle_cos[i - 180]) * kZ2) + ((-angle_sin[i - 180]) * kX2) + pZ2;
        deviation1 = deviation13;
      } else {
        float deviation14 =
            ((angle_cos[360 - i] * kX2) - ((-angle_sin[360 - i]) * kZ2)) + pX2;
        deviation2 =
            (angle_cos[360 - i] * kZ2) + ((-angle_sin[360 - i]) * kX2) + pZ2;
      ...
      if (buffer[1] == 0x31 && buffer[2] == 0x38) {
        if (buffer[4] == 100) {
          ...
          if (buffer[6] == 1) {
            ...
            pthread_mutex_lock(&acc_mutex);
            acc_paused = false;
            pthread_cond_signal(&acc_cond); // Signal the condition variable to
                                            // wake up the thread
            pthread_mutex_unlock(&acc_mutex);
